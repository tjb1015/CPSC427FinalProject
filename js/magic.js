...
//makes grid but has it hidden, so it is always on the bottom
    ...
    var G = document.getElementById('griddy')
    ...
    for (var i=0;i<finhgt;i++){
        ...
        yT.setAttribute("x",i==0? 11:0)
        yT.setAttribute("y",15+i*100)
        yT.textContent = i==0? ",0":i * 100;
        G.appendChild(yT)
...
//Grid is the first thing called so it is the bottom element, this function simply hides and unhides it.
    function clearGrid(grid) {
        var dd = document.getElementById("griddy");
        if (grid == 'yes') {
...
function drawItem() {
    ...
    svg.onmousedown = function (evt) {
        document.getElementById("drawArea").appendChild(path)
        path.drawIt
